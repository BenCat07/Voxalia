        ...
        public MaterialImage GenerateSeedImageBmp(WorldSystem.Region tregion, int minx, int miny, int maxx, int maxy, int wid)
        {
            ...
            for (int x = 0; x < wid; x++)
            {
                for (int y = 0; y < wid; y++)
                {
                    ...
                    Biome b = tregion.Generator.GetBiomeGen().BiomeFor(tregion.TheWorld.Seed2, tregion.TheWorld.Seed3, tregion.TheWorld.Seed4, minx + (maxx - minx) * x * one_div_wid, miny + (maxy - miny) * y * one_div_wid, 0, h);
                    Material renderme;
                    if (h > 0)
                    {
