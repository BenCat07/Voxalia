...
void main()
{
	...
	fi.texcoord = vec3(f[0].texcoord.z, f[0].texcoord.w, f[0].tcind);
	EmitVertex();
	// Second Vertex
	gl_Position = proj_matrix * vec4(f[0].screenrect.x, f[0].screenrect.w, 0.0, 1.0);
	fi.texcoord = vec3(f[0].texcoord.x, f[0].texcoord.w, f[0].tcind);
	EmitVertex();
	// Third Vertex
	gl_Position = proj_matrix * vec4(f[0].screenrect.z, f[0].screenrect.y, 0.0, 1.0);
	fi.texcoord = vec3(f[0].texcoord.z, f[0].texcoord.y, f[0].tcind);
	EmitVertex();
	// Fourth Vertex
	gl_Position = proj_matrix * vec4(f[0].screenrect.x, f[0].screenrect.y, 0.0, 1.0);
	fi.texcoord = vec3(f[0].texcoord.x, f[0].texcoord.y, f[0].tcind);
	EmitVertex();
	EndPrimitive();
}
